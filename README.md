...
- **技術デモ** — 自分の文書を追加して検索を試す、取得文書、類似度、応答時間、プロンプト、評価データセットの実行結果を表示
...
| **RAG技術デモ** | 自分の文書（.md/.txt/.html）を追加して検索を試す、インデックス情報（文書数・チャンク数・チャンクサイズ・オーバーラップ）、検索の実行、取得文書とスコア、組み立てられたプロンプト、評価データセットの一括実行 |
...
**B. サイドバーからキーを入力する**（他人にデモを見せる場合）

サイドバーの「🔑 自分のAPIキーで試す」から、自分のキーをその場で貼り付けられます。
`.env` を編集する必要はありません。挙動は次のとおりです。

- キーは `st.session_state`（このブラウザセッションの中だけ）に保持され、`.env` やディスクには一切書き込まれません
- タブを閉じる、または再読み込みすると失われます
- 入力するとその場で `.env` の `APP_MODE` に関わらずクラウドモードに切り替わります。空にすると解除されます
- サーバー側の実装は、キーごとに別々の OpenAI クライアントをキャッシュしています。この方式を採らずキーをプロセス全体で共有すると、
  同じサーバーを複数人が同時に開いたときに他人のキーで通信してしまう恐れがあるため、これを避けています

一方で、検索用の埋め込みインデックス（`Retriever`）はモード単位でキャッシュしていて、
セッション単位ではありません。同じサーバーをクラウドモードで複数人が使っている場合、
埋め込みの計算は最初にクラウドモードで検索した人のキーで実行され、
その結果（ベクトルの数値のみで鍵情報は含みません）は以降の利用者と共有されます。
自分のPCだけでローカルに動かす分には関係ありません。
...
| `test_llm_client.py` | セッションごとのAPIキーでOpenAIクライアントが混線しないこと |
| `test_views_shared.py` | 検索インデックスのキャッシュがモード単位で分かれていること |
| `test_app_views.py` | 画面遷移の統合テスト（Streamlit `AppTest`）。HTMLエスケープ漏れ・不正な`scenario_id`・アップロード機能の回帰を含む |
...
コアロジック（`src/`）を一通りテストした後、画面（`views/`）を実際にクリックして見直したところ、
ロジック層のテストだけでは検出できない問題が4件見つかりました。

4. **評価画面が、担当者自身の入力とAI生成の要約を無害化せずHTMLとして描画していた**
   （[review.py](views/review.py)）。`unsafe_allow_html=True` と組み合わせて未エスケープの
   文字列を埋め込んでおり、「顧客への応対」欄に `<img src=x onerror=...>` のようなHTMLを打つと
   評価画面でそのまま実行されていた。技術デモ画面 (`tech_demo.py`) では同じ状況で
   `escape_for_display()` を使っており、そちらとの不整合でもあった。同じ関数で揃えて修正。
5. **セッションの `scenario_id` が解決できないと、生の例外画面に落ちていた。** 他の全画面が
   `show_error()` でエラーを利用者向けの文言に変換しているのに、この経路だけ素通しだった。
   try/except で包み、シナリオ選択へ戻す導線を追加。
6. **アップロードの検証ロジック（拡張子・サイズ・HTML無害化）が実装・テスト済みなのに、
   呼び出す `st.file_uploader` がどこにもなかった。** 死んでいたコードをRAG技術デモ画面に接続し、
   「自分の文書を追加して検索を試す」機能として完成させた。ラベル付き評価データセットは
   常に同梱文書だけを対象にする（追加した文書でスコアの意味がぶれないように分離）。
7. **画面遷移の統合テストが1件も無かった。** 上の4・5はまさにこの隙間で見逃されていた。
   Streamlit標準の `AppTest`（追加依存なし）でボタン操作とセッション遷移を検証する
   `tests/test_app_views.py` を追加し、4・5・6を回帰テストとして固定した。
